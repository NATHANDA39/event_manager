...
def show_signin_page():
    ...
    with ui.element("div").classes("w-full h-screen flex flex-row m-0 p-0 gap-0 overflow-hidden"):
        ...
        with ui.column().classes('w-1/2 h-full bg-white flex flex-col justify-center items-center p-4'):
            ...
            # Input fields
            ui.input('YOUR EMAIL', placeholder='Enter your email').classes('w-1/2').props("outlined dense")
            ui.input('PASSWORD', placeholder='Enter your password', password=True, password_toggle_button=True).classes('w-1/2').props("outlined dense")
            ...
            ui.button('Sign In', icon='fa-solid fa-arrow-right-to-bracket').classes('w-1/4 text-white font-bold rounded-sm shadow').props("flat dense no-caps").classes('bg-purple-600 w-[50%] p-2')
            ui.label('Or').classes('text-gray-500')
            ui.button('Sign in with Google', icon='fa-brands fa-google').classes("bg-purple-600 text-white w-[50%] p-2").props("flat dense no-caps")
            ...
            with ui.column().classes("items-center"):
                ...
                with ui.link("", "/signup").classes("no-underline"):
                    with ui.button(text=None).classes('relative px-4 flex items-center justify-center bg-transparent border-none'):
                        ...
                        ui.label('Signup').props("flat dense no-caps").classes('relative z-10 text-white font-semibold text-sm')